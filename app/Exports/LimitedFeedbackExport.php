...
class LimitedFeedbackExport implements FromQuery, WithMapping, WithHeadings, ShouldAutoSize
{
    ...
    public function map($session): array
    {
        ...
        $rows = [
            $this->buildResponseRow($session, $session->responses->first(), false, true),
        ];

        // Subsequent rows should not contain session metadata
        foreach ($session->responses->skip(1) as $response) {
            array_push(
                $rows, $this->buildResponseRow($session, $response, true, true)
            );
        }
